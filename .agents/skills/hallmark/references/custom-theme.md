...
Catalog entries continue to record `theme: <name>` and skip `theme_axes` (the catalog's three axis values are stated in the rotation block when picking — see [`SKILL.md`](../SKILL.md) § Theme-diversification rule). Step 2.5 logic uses the same diversification check on both — for catalog entries it reads the axes from the stated pick; for custom entries it reads them from the entry.
...
2. **Does not save themes for reuse.** A custom run is per-output. The skill does not write back to a shared catalog file. If the user wants a permanent theme, they paste the custom palette into their project's `tokens.css` themselves and name it.
